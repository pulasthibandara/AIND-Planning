...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            pn_a = PgNode_a(action)
            # test if pn_a has prenodes that are sa subset of s level
            if pn_a.prenodes.issubset(self.s_levels[level]):
                a_nodes.append(pn_a)
                for pn_s in self.s_levels[level]:
                    # connect instances
                    pn_s.children.add(pn_a)
                    pn_a.parents.add(pn_s)
    ...
    def add_literal_level(self, level):
        ...
        for pn_a in self.a_levels[level-1]:
            for node in pn_a.effnodes:
                pn_a.children.add(node)
                node.parents.add(pn_a)
                nodes.add(node)
    ...
    def h_levelsum(self) -> int:
        ...
        for level in self.s_levels:
            for state in level:
                if state.symbol in self.problem.goal and\
                    state not in states and\
                    state.is_pos:
